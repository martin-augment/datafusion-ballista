...
/// Float64)` today — so the filter is symmetric with the writer's placement
/// decision.
#[derive(Clone, Debug)]
...
pub struct ExchangeExec {
    ...
    /// range-repartition op (URRE or ORRE). Written by
    /// `AdaptiveExecutionGraph::maybe_range_repartition_overlap_remap` when
    ...
    /// to build `PerPartitionFilterExec` predicates for downstream stage
    /// `N+1`.
    ///
    /// `None` on any exchange that isn't downstream of a range repartition
    /// (the common case — hash/range/round-robin exchanges never populate
    /// this).
    ...
    fn with_new_children(
        ...
        if children.len() == 1 {
            let new_exec = Self::new_with_details(
                ...
                self.shuffle_partitions.clone(),
                self.coalesce.clone(),
                // Same rationale for range-repartition routing: transform
                // passes that rebuild the parent must not drop range
                // boundaries the scheduler already parked on this exchange.
                self.range_repartition_routing.clone(),
                self.broadcast,
                self.inactive_stage,
            );

...
fn restrict(
    ...
    // indices in the same order — otherwise the operator's
    // `predicates.len() == child.partition_count()` invariant breaks and
    // even if it didn't, task-local partition `j` would be filtered
    // through the wrong global predicate. Under `under_collect`, neither
    // child nor predicates get sliced and the generic path below applies.
    if !under_collect && let Some(ppf) = plan.downcast_ref::<PerPartitionFilterExec>() {
        let child = plan.children()[0].clone();
        let new_child = restrict(child, partitions, false)?;
        let new_predicates: Vec<_> = partitions
            .iter()
            .map(|&p| ppf.predicates()[p].clone())
            .collect();
